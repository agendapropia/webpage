...
class CreatePermissionTables extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::connection('mysql_permissions')->create(
            $tableNames['permissions'],
            function (Blueprint $table) {
                $table->bigIncrements('id');
                $table->string('name'); // For MySQL 8.0 use string('name', 125);
                $table->string('description', 250)->nullable();
                $table->string('module_id')->default(1)->comment('Se relaciona con la tabla de permission_modules');
    ...
    public function down()
    {
        ...
        Schema::connection('mysql_permissions')->drop(
            $tableNames['role_has_permissions']
        );
        Schema::connection('mysql_permissions')->drop(
            $tableNames['model_has_roles']
        ...
        Schema::connection('mysql_permissions')->drop($tableNames['roles']);
        Schema::connection('mysql_permissions')->drop(
            $tableNames['permissions']
        );
    }
